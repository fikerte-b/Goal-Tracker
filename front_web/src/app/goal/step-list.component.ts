...
@Component({
  ...
  template: `
 <div class="container">
      <div style="border: 2px solid black; background-color: MintCream;" class="card" >
        <div class="card-content" >
          <p class="title" > {{index + 1}}. {{ step.title }}</p>
          <hr />
          <p><strong>Description:</strong> {{ step.description }}</p>
          <p><strong>Status:</strong>  {{ step.status }}</p>
          <p [appPastDue]="dueDate"><strong>Deadline:</strong>  {{ step.deadline }} days</p>
          <p><strong>Date Created:</strong>  {{ step.dateCreated | date: 'dd-MM-yyyy' }}</p>
          <p><strong>Todays Date:</strong>  {{ dateNow | date: 'dd-MM-yyyy' }}</p>
        </div>
        <footer class="card-footer" style="background-color: black;">
        
          <p class="card-footer-item">
            <span>
              <a  [routerLink]="['', 'goals', goalId, 'steps', step._id]" style="color: white">Edit</a>
            </span>
          </p>
          <p class="card-footer-item">
            <a (click)="deleteHandler(step._id)" style="color: white"> Delete </a>
          </p>
        </footer>
      </div>
    </div>
